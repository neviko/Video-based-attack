function [shape,v_points] = imageAccDetection(image_orig)
clc
...
for i=1:size(bboxs,2)
    ...
    cd(lastDir); %get back to the last directory
    
    % Draw the mask
    %changed_frame=getframe; % get frame is a struct that contain the image with the points on it
    %editedImage=changed_frame.cdata; % cdata is the image with the points
end
